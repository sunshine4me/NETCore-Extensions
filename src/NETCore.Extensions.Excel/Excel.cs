...
namespace NETCore.Extensions.Excel {
    public class ExcelStream : IExcelStream, IDisposable {
        ...
        public ExcelStream Load(Stream stream) {
            ...
            _file = null;
            ZipArchive = new ZipArchive(stream, ZipArchiveMode.Read | ZipArchiveMode.Update, true);
            ReadFromZip();
        ...
        /// <summary>
        /// 内存中创建ExcelStream (文件最大1MB)
        /// </summary>
        /// <returns></returns>
        public ExcelStream Create(MemoryStream ms) {

            ms.Write(NewExcel.Bytes, 0, NewExcel.Bytes.Length);
        ...
        /// <returns></returns>
        public SheetWithoutHDR LoadSheet(uint Id) {
            var worksheet = WorkBook.Where(x => x.Id == Id).First();
            var e = ZipArchive.GetEntry($"xl/{worksheet.Target}");
            using (var stream = e.Open()) {
                var sr = new StreamReader(stream);
                var result = sr.ReadToEnd();
                return new SheetWithoutHDR(worksheet, result, this, CachedSharedStrings);
            }
        }
        ...
        /// </summary>
        /// <param name="Id">Sheet名称对应的Id</param>
        public void RemoveSheet(uint Id) {
        ...
        private uint createSheet(string name) {
